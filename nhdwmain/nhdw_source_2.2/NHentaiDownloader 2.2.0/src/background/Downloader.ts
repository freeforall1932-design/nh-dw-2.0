...
export default class Downloader
{
    ...
    async #downloadPageInternalAsync(currPage: number, progress: number) {
        ...
        // Get a random image server from the available servers (i1-i4.nhentai.net)
        let imageserverID = Math.floor(Math.random() * 4) + 1; // Pick a random image server ID 1-4
        let imageserverURL = `https://i${imageserverID}.nhentai.net/`; // Base image server URL
        
        // v2 API returns path as "galleries/9/1.jpg", so we use it directly
        // The media_id is used in the old format: galleries/{media_id}/{page}.{ext}
        let imagePath = this.#mediaId + '/' + filenameParsing; // Old format for backwards compatibility

        if (this.useZip !== "raw") { // ZIP (or equivalent) format
            const resp = await fetch(imageserverURL + 'galleries/' + imagePath);
            if (resp.ok)
            {
            ...
            chrome.downloads.download({
                url: imageserverURL + 'galleries/' + this.#mediaId + '/' + filenameParsing,
