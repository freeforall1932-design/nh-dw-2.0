...
function run(options) {
    ...
    const chromeStub = {
        storage: {
            sync: {
                // StorageArea.get answers ONLY the requested keys; a
                // whole-store merge mock hides that rule (and with it every
                // optional key a reader forgot to ask for - item 59).
...
(async () => {
    ...
    // The bar's picker and every card/bar job must show and dispatch the SAVED
    // list format; an unset key still inherits the single-title format. Each
    // case runs the real bundle against a storage stub that returns only the
    // keys the bundle actually requested.
    {
        const selectedFormat = (ctx) => {
            const select = ctx.dom.document.getElementById("nhdw-format");
            if (!select) {
                fail("the bar must offer the format picker");
            }
        ...
        for (const entry of formatCases) {
            const ctx = run({ settings: entry.stored });
            await wait(0);
            assert.deepEqual(selectedFormat(ctx), [entry.expected], entry.label + ": selected bar option");
            assert.deepEqual(ctx.syncWrites, [], entry.label + ": opening the page is read-only");
            assert.deepEqual(ctx.settings, entry.stored, entry.label + ": no normalization written back");
            ...
            assert.ok(job, entry.label + ": card dispatch");
            assert.equal(job.formatOverride, entry.expected, entry.label + ": card format");
            assert.equal(job.separate, true, entry.label + ": a single card is one file");
            ...
            assert.ok(barJob, entry.label + ": bar dispatch");
            assert.equal(barJob.formatOverride, entry.expected, entry.label + ": bar format");
            assert.equal(barJob.separate, true, entry.label + ": separate files");
            assert.deepEqual(Object.keys(barJob.allDoujinshis), ["111111", "222222"]);
            assert.deepEqual(ctx.syncWrites, [], entry.label + ": downloads do not materialize defaults");
        }
        console.log("PASS: card/bar controls display and dispatch all " + formatCases.length + " explicit/inherited/legacy list formats without writes (item 59)");

        // An explicit edit persists on its own key and survives a reload,
        // without touching the single-title format or unrelated keys.
        ...
        console.log("PASS: list bar format edits persist independently and survive a page reload (item 59)");
    }
