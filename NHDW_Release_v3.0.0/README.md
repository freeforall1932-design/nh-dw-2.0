...
- **In-page card controls (3.4.0, all six sites in 3.10.0)** — Every gallery card on a listing page gets a **Download** button, a **Select** box and a bookmark icon, with a floating bar showing `N selected -> format -> output -> Download` plus **Select all**. The bar is visible whenever the page has listing cards, and each site's own card markup is matched by its own selector row (nhentai, hentaiera, imhentai, hentaienvy, hentaifox, hitomi). Selection is shared with the panel in both directions and survives infinite scroll. Toggle: Settings -> *Download / Select buttons on listing cards*.
...
Firefox (desktop and Android) is fully supported via the separate `NHDW_Firefox_v1.0.0` package (v1.4.0). It uses Firefox MV3 event pages (`background.scripts`), the website-embedded header UI drawer, and full parity with Chrome's multi-site download pipeline. See [`NHDW_Firefox_v1.0.0/README.md`](../NHDW_Firefox_v1.0.0/README.md).
