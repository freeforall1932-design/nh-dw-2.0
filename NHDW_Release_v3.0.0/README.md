...
- **In-page card controls (3.4.0)** — Every gallery card on a listing page gets a **Download** button and a **Select** box, with a floating bar showing `N selected -> format -> output -> Download`. Selection is shared with the panel in both directions and survives infinite scroll. Toggle: Settings -> *Download / Select buttons on listing cards*.
...
```bash
npm test                          # 389 unit tests (offline)
