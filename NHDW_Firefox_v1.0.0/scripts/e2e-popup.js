...
//      invalid-page notice.
//   14. listing bootstrap + panel Save offline (item 40): a listing TAB URL
...
// merge the whole store over defaults (that masked the item-59 missing key). Since item 40 the
// scripting stub also answers the two real bootstraps: a js/getGalleries.js
// injection gets the armed listing payload, and readGalleryFromTab's poll gets
// the armed tab gallery.
//
...
// recorded-history entry through the local store. isDownloadFinished must
// answer true or bootstrap stops at updateProgress and never re-renders.
function setPreviewTabUrl(url) { sourceTab.url = url; isDownloadFinishedAnswer = true; }
function setStoredHistory(history) { localStore.downloadHistory = history; }
...
(async () => {
    ...
    // ---- Phase 15: listing bootstrap + panel Save offline (item 40) --------
    ...
    console.log("PASS phase 15a: a listing tab URL bootstraps the view through the injected content script (item 40)");
    ...
    console.log("PASS phase 15b: a gallery tab URL bootstraps the preview from the tab's own metadata (items 40/62a)");
    ...
    console.log("PASS phase 15c: the panel's Save offline sends one list-mode job for the tabbed gallery (item 62a)");
    ...
    console.log("PASS phase 15d: Save offline asks before re-saving a recorded title and forces exactly that gallery");
