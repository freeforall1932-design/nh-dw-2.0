// Minimal dependency-free PDF writer for the PDF output format (backlog item
// 16). Produces a PDF 1.4 document with one page per image, embedding each
// page as a JPEG (DCTDecode) XObject at its native size — no re-encode for
// baseline/progressive RGB JPEGs, which keeps assembly fast and lossless.
//
// Design constraints:
//   - Pure TypeScript, no DOM/chrome dependencies, so it runs in the offscreen
//     document, the MV3 service worker fallback, and plain Node test builds.
//   - Callers pre-encode non-JPEG pages (PNG/GIF/WebP, or CMYK JPEGs) to RGB
//     JPEG before calling buildPdfDocument; see Downloader.#preparePdfImage,
//     which uses createImageBitmap + OffscreenCanvas where available.
//
// Structure produced:
...
export type PdfImage = {
    bytes: Uint8Array;  // complete JPEG file (SOI..EOI)
    width: number;      // pixels
...
export type JpegInfo = {
    width: number;
    height: number;
    components: number; // 1 = grayscale, 3 = YCbCr RGB, 4 = CMYK/YCCK
};
...
export function buildPdfDocument(images: PdfImage[]): Uint8Array {
    ...
    const pushBinary = (asBytes: Uint8Array) => {
        parts.push(asBytes);
        length += asBytes.length;
    };
